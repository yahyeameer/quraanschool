    ...
    return (
        ...
        >
            <div className="flex flex-row items-center gap-4 mb-6">
                <Avatar className="h-12 w-12 border-2 border-indigo-100 dark:border-indigo-500/30">
                    <AvatarImage src={img} alt={name} />
                    <AvatarFallback className="bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-200">{name[0]}</AvatarFallback>
                </Avatar>
                <div className="flex flex-col">
                    <figcaption className="text-base font-bold text-foreground dark:text-slate-100 group-hover:text-primary dark:group-hover:text-white transition-colors">
                        {name}
                    </figcaption>
                    <p className="text-xs font-medium text-muted-foreground dark:text-slate-400 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">
                        {role}
                    </p>
                </div>
                <div className="ml-auto flex items-center gap-0.5 text-amber-500 dark:text-amber-400 bg-amber-100/50 dark:bg-amber-500/10 px-2 py-1 rounded-full border border-amber-200 dark:border-amber-500/20">
                    <Star className="h-3 w-3 fill-current" />
                    <span className="text-xs font-bold ml-1">5.0</span>
                </div>
            </div>

            <blockquote className="text-sm leading-relaxed text-slate-600 dark:text-slate-300 group-hover:text-slate-800 dark:group-hover:text-slate-200 transition-colors relative z-10">
                "{body}"
            </blockquote>
            ...
            <div className="absolute top-6 right-6 opacity-5 dark:opacity-10 group-hover:opacity-10 dark:group-hover:opacity-20 transition-opacity">
                <Quote className="h-8 w-8 text-black dark:text-white fill-current" />
            </div>

            {/* Gradient Glow */}
            <div className="absolute inset-0 bg-gradient-to-tr from-indigo-500/5 dark:from-indigo-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none" />
        </figure>
    );
};
        ...
        <section className="py-32 relative overflow-hidden bg-slate-50 dark:bg-slate-950 border-t border-slate-200 dark:border-white/5 transition-colors duration-500">
            ...
            <div className="container mx-auto px-4 relative z-10 mb-20 text-center">
                <motion.div
                    ...
                    className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-100 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 text-sm font-bold mb-6 border border-indigo-200 dark:border-indigo-500/20 shadow-glow"
                >
                ...
                <motion.h2
                    ...
                    viewport={{ once: true }}
                    className="font-amiri text-4xl md:text-5xl font-bold mb-6 tracking-tight text-foreground dark:text-white"
                >
                    Trusted by <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-cyan-500 dark:from-indigo-400 dark:to-cyan-400">Communities</span>
                </motion.h2>
                <motion.p
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    ...
                    viewport={{ once: true }}
                    className="text-muted-foreground dark:text-slate-400 text-lg max-w-2xl mx-auto font-light"
                >
            ...
            <div className="relative flex w-full flex-col items-center justify-center overflow-hidden">
                ...
                {/* Fade Edges */}
                <div className="pointer-events-none absolute inset-y-0 left-0 w-1/4 bg-gradient-to-r from-slate-50 dark:from-slate-950 to-transparent z-10"></div>
                <div className="pointer-events-none absolute inset-y-0 right-0 w-1/4 bg-gradient-to-l from-slate-50 dark:from-slate-950 to-transparent z-10"></div>
            </div>
        </section>
    );
}
